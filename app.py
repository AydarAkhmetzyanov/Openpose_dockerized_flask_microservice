...
def run_demo(net, image_provider, height_size, cpu):
    ...
    results['scale'] = scale
    results['pad'] = pad
    results['pose_entries'] = pose_entries
    results['all_keypoints'] = all_keypoints
...
def upload():
    if request.method == 'POST':
        ...
        return json.dumps({'filename':f_name, 'humans': run_demo(net, image, 256, True)}, cls=NumpyEncoder)
